...
for team_id in range(1610612737, 1610612767): #for all normal features
  ...
  for index, row in cdata.iterrows():
    ...
    if(home_team==team_id):
      ...
      if index not in rows_to_drop:
        pdata['PTS_away:'].append(str(pts))
        pdata['REB_away:'].append(str(rbds))
        pdata['AST_away:'].append(str(asts))
        pdata['FG_PCT_away:'].append(str(fg))
        pdata['FT_PCT_away:'].append(str(ft))
        pdata['FG3_PCT_away:'].append(str(fg3))
      rounds+=1
...
for team_id in range(1610612737, 1610612767): #again for margin
  ...
  for index, row in cdata.iterrows():
    ...
    if(home_team==team_id):
      ...
      if index not in rows_to_drop:
        pdata['MARGIN_home:'].append(str(margin))
      rounds+=1
...
with open(filename, 'w', newline='') as csvfile:
    ...
    writer.writerows(zip(*pdata.values())) # Write rows
